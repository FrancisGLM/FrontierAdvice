// ============================================================
// MODELO HÍBRIDO v6.2 (FAST) — FrontierAdvice Predicción
...
function stats(arr) {
  ...
  const std = Math.sqrt(
    valid.reduce((s, v) => s + Math.pow(Number(v) - mean, 2), 0) / n,
  );
  return { mean, std, n };
}
...
const hist = estados
  .map((e) => {
    ...
    const motivo = (e?.motivo_estado || e?.mensaje_original || "")
      .toString()
      .toLowerCase();
...
function calcHistoryScore(motivos) {
  ...
  if (cierres24h >= 1) {
    score += 22;
    motivos.push("cierre en las últimas 24h");
  } else if (cierres3d >= 2) {
    score += 16;
    motivos.push("múltiples cierres en 3 días");
  } else if (cierres7d >= 3) {
    score += 12;
    motivos.push("cierres frecuentes en 7 días");
  } else if (cierres7d >= 1) {
    score += 6;
  ...
  if (prec7d >= 3) {
    score += 8;
    motivos.push("precauciones frecuentes");
  } else if (prec7d >= 1) {
    score += 4;
    motivos.push("antecedentes de precaución");
  }
  if (ultimoEstado === "cerrado") {
    score += 18;
    motivos.push("último estado: cerrado");
  } else if (ultimoEstado === "precaucion") {
    score += 10;
    motivos.push("último estado: precaución");
  }
...
const dias = [
  { h: 24, d: forecast[0] || null, horizonWeight: 1.0, fecha: addDays(hoy, 1) },
  {
    h: 48,
    d: forecast[1] || null,
    horizonWeight: 0.85,
...
for (const item of dias) {
  ...
  if (viento >= 100) {
    meteoScore += 30;
    hardSignals++;
    motivos.push("viento extremo (>100 km/h)");
  } else if (viento >= 80) {
    meteoScore += 22;
    motivos.push("viento muy fuerte (80-100 km/h)");
  } else if (viento >= 60) {
    meteoScore += 14;
    motivos.push("viento fuerte (60-80 km/h)");
  } else if (viento >= 40) {
    meteoScore += 7;
    motivos.push("viento moderado (40-60 km/h)");
  }

  if (precip >= 20) {
    meteoScore += 14;
    motivos.push("precipitación muy alta");
  } else if (precip >= 12) {
    meteoScore += 10;
    motivos.push("precipitación alta");
  } else if (precip >= 5) {
    meteoScore += 6;
    motivos.push("precipitación moderada");
  } else if (precip >= 2) {
    meteoScore += 3;
    motivos.push("precipitación leve");
  }

  if (tempMin <= -10) {
    meteoScore += 14;
    motivos.push("frío extremo (<-10°C)");
  } else if (tempMin <= -5) {
    meteoScore += 10;
    motivos.push("temperatura muy baja (-10 a -5°C)");
  } else if (tempMin <= 0) {
    meteoScore += 6;
    motivos.push("temperatura bajo cero");
  } else if (tempMin <= 2) {
    meteoScore += 3;
    motivos.push("temperatura cercana a cero");
  }
  ...
  if (anomTempSigma !== null && anomTempSigma <= -2.0) {
    anomaliaScore += 12;
    hardSignals++;
    motivos.push("temperatura anormalmente baja (>2σ)");
  } else if (anomTempSigma !== null && anomTempSigma <= -1.5) {
    anomaliaScore += 8;
    motivos.push("temperatura significativamente bajo lo normal");
  } else if (anomTempSigma !== null && anomTempSigma <= -1.0) {
    anomaliaScore += 4;
    motivos.push("temperatura bajo lo normal");
  }
  ...
  if (anomPrecipSigma !== null && anomPrecipSigma >= 2.0) {
    anomaliaScore += 10;
    hardSignals++;
    motivos.push("precipitación anormalmente alta (>2σ)");
  } else if (anomPrecipSigma !== null && anomPrecipSigma >= 1.5) {
    anomaliaScore += 6;
    motivos.push("precipitación significativamente sobre lo normal");
  } else if (anomPrecipSigma !== null && anomPrecipSigma >= 1.0) {
    anomaliaScore += 3;
    motivos.push("precipitación sobre lo normal");
  }

  const anomVientoSigma = anomaliaSigma(viento, stViento.mean, stViento.std);
  if (anomVientoSigma !== null && anomVientoSigma >= 2.0) {
    anomaliaScore += 8;
    motivos.push("viento anormalmente alto (>2σ)");
  } else if (anomVientoSigma !== null && anomVientoSigma >= 1.5) {
    anomaliaScore += 4;
    motivos.push("viento sobre lo normal");
  }

  const diasPeligrosos = historicoDia.wmos.filter((w) =>
    WMO_PELIGROSOS.has(w),
  ).length;
  const pctEstacional =
    historicoDia.wmos.length > 0
      ? diasPeligrosos / historicoDia.wmos.length
      : 0;

  if (pctEstacional >= 0.6) {
    anomaliaScore += 8;
    motivos.push("semana de alto riesgo estacional histórico");
  } else if (pctEstacional >= 0.35) {
    anomaliaScore += 4;
    motivos.push("semana de riesgo estacional moderado");
  }
  ...
  if (altitud >= 3000) {
    contextScore = 12;
    motivos.push("paso sobre 3000 m.s.n.m.");
  } else if (altitud >= 2500) {
    contextScore = 8;
    motivos.push("paso de alta montaña (>2500 m)");
  } else if (altitud >= 1500) {
    contextScore = 4;
    motivos.push("paso de montaña media");
  }
  ...
  if (ultimoEstado === "cerrado") {
    if (isCierreEstructural)
      hardSignals += 3; // Fuerza cierre seguro
    else hardSignals++;
  }

  // ── 5. Agregación ponderada (Sin Alertas) ──────────────────────────────────
  ...
  else if (forecast.length >= 3 && nHistDias >= 2 && hist.length >= 5)
    confidence = "alta";
  ...
  out.push({
    json: {
      ...
      modelo_version: "risk-v6.2-hybrid-fast",
      ...
      datos_entrada: {
        ...
        historico_temp_media:
          stTempMin.mean !== null ? Number(stTempMin.mean.toFixed(1)) : null,
        historico_precip_media:
          stPrecip.mean !== null ? Number(stPrecip.mean.toFixed(1)) : null,
        anomalia_temp_sigma:
          anomTempSigma !== null ? Number(anomTempSigma.toFixed(2)) : null,
        anomalia_precip_sigma:
          anomPrecipSigma !== null ? Number(anomPrecipSigma.toFixed(2)) : null,
      },
    },
  });
}
